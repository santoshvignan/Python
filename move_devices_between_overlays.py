...
def device_ssh_connection(device_mgmt_ip,device_username,device_password):
    #Logging into the device and get the old device information
    global device_chassis_number
    global device_serial_number
    global device_system_ip
...
def connect_to_vmanage(has_device_been_invalidated_in_old_overlay,vmanage_ip,vmanage_username,vmanage_password):
    ...
    if login.status_code == 200:
        print("Successfully logged into vManage %s.." %vmanage_ip)
    ...
    if (has_device_been_invalidated_in_old_overlay == False):
        print ("Moving the device to invalid state...")
        move_device_to_invalid_state_url = login_url + "dataservice/certificate/save/vedge/list"
        print(move_device_to_invalid_state_url)
        ...
        if move_device_to_invalid_state.status_code == 200:
            ...
            if send_to_controllers.status_code == 200:
                ...
                has_device_been_invalidated_in_old_overlay = True
                return has_device_been_invalidated_in_old_overlay

    if (has_device_been_invalidated_in_old_overlay == True):
        ...
        for device in devicelist_data_json["data"]:
            if device["uuid"] == device_chassis_number:
                ...
                if device["validity"] == 'valid':
                    print ("Device is in valid State.")
                else:
                    ...
                    if move_device_to_valid_state.status_code == 200:
                        ...
                        send_to_controllers = requests.post(send_to_controllers_url,headers=post_header,verify=False)
                        if send_to_controllers.status_code == 200:
                            print ("Device has moved to valid state.")
...
if __name__ == "__main__":
    ...
    device_ssh_connection(device_mgmt_ip,device_username,device_password)
    has_device_been_invalidated_in_old_overlay = False
    has_device_been_invalidated_in_old_overlay = connect_to_vmanage(has_device_been_invalidated_in_old_overlay,old_vmanage_ip,vmanage_username,vmanage_password)
    if (has_device_been_invalidated_in_old_overlay == True):
        connect_to_vmanage(has_device_been_invalidated_in_old_overlay,new_vmanage_ip,vmanage_username,vmanage_password)
