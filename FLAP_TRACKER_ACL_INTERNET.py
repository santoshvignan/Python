...
f = open("/home/vignan/internet-router-tracker.txt",'w')
...
tn.write(('interface GigabitEthernet0/0.235'+'\r').encode('ascii'))
...
for i in range(0,100000000000):
    ...
    tn.write(('ip access-group tracker_block in'+'\r').encode('ascii'))
    d = tn.read_until(b'internet(config-subif)#',timeout=2)
    #print (d.decode('ascii'))

    time.sleep(10)

    tn.write(('no ip access-group tracker_block in'+'\r').encode('ascii'))
